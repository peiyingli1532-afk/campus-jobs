...
def call_deepseek(current, news, today):
    system = (
        ...
        "- hidden=true 的公司要保持隐藏，绝对不要自动解除隐藏（脚本无法可靠核实深圳是否有非技术全职校招岗）；这些公司的恢复只能由人工核实后处理；\n"
        "- 新增公司要设 addedDate 为今天；已有公司(含 hidden 的)的 addedDate 保持不变；\n"
        "- 已有公司的 url、jd、publishDate、hidden 字段要原样保留不变（不要改动或截断已有链接，不要改动已有发布时间）；新增公司时要为它的每个岗位方向生成 jd 关键词画像(8~15个空格分隔关键词，结合该公司业务特点)，且 url 必须是完整可访问的官方投递链接(完整域名+路径，不能截断)；\n"
